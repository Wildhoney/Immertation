...
/**
 * A record with string or number keys and unknown values.
 */
export type Object = Record<string | number, unknown>;

/**
 * An array with unknown element values.
 */
export type Array = unknown[];

/**
 * An object with symbol keys and unknown values.
 */
export type SymbolObject = Record<symbol, unknown>;

/**
 * An object with string or symbol keys and unknown values.
 */
export type PropertyObject = Record<string | symbol, unknown>;
...
/**
 * A path representing a sequence of property keys and array indices.
 */
...
export class Annotation<T> {
  ...
  /**
   * Assigns an annotation to a new process identifier.
   *
   * @template T - The value type
   * @param annotation - The annotation to assign
   * @param process - The new process identifier
   * @returns A new annotation with the first task's data and the new process
   */
  static assign<T>(annotation: Annotation<T>, process: Process): Annotation<T> {
    const task = annotation.tasks[0];
    return Annotation.new(task.value, task.state, process, task.property);
  }
...
/**
 * Represents any object or array that can be used as a model.
 * Alias for Objectish from Immer.
 */
